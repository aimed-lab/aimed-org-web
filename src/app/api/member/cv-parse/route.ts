...
/**
 * POST /api/member/cv-parse
 * Upload a CV PDF and extract all content types using Gemini.
 */
...
Return ONLY the JSON object. No markdown fences, no explanations.`
    ...
    // Check talks for duplicates (by title similarity)
    ...
    const talkResults = parsed.talks.map((t) => {
      ...
      const dup = existingTalks.find(
        (et) => et.title.toLowerCase().replace(/[^a-z0-9\s]/g, "").trim() === norm
      )
    ...
    const honorResults = parsed.honors.map((h) => {
      ...
      const dup = existingHonors.find(
        (eh) => eh.awardName.toLowerCase().replace(/[^a-z0-9\s]/g, "").trim() === norm
      )
    ...
    const softwareResults = parsed.software.map((s) => {
      ...
      const dup = existingSoftware.find(
        (es) => es.name.toLowerCase().replace(/[^a-z0-9\s]/g, "").trim() === norm
      )
    ...
    const patentResults = parsed.patents.map((p) => {
      ...
      const dup = existingPatents.find(
        (ep) => ep.title.toLowerCase().replace(/[^a-z0-9\s]/g, "").trim() === norm
      )
...
/**
 * PUT /api/member/cv-parse
 * Import selected items from a parse result into the database.
 */
      ...
      for (const talk of talks as ParsedTalk[]) {
        await prisma.talk.create({
          data: {
            ...
            host: talk.host || null,
            city: talk.city || null,
            country: talk.country || null,
            date: talk.date ? new Date(talk.date) : null,
            talkType: talk.talkType || null,
          },
        })
        counts.talks++
      }
      ...
      for (const patent of patents as ParsedPatent[]) {
        await prisma.patent.create({
          data: {
            ...
            filingInfo: patent.filingInfo || null,
          },
        })
        counts.patents++
      }
